...
if [ "$ALL_GOOD" = true ]; then
    ...
    echo ""
    echo "Next steps:"
    echo "  1. Edit config.yaml with your preferences (pm.tool, repo.type)"
    echo "  2. Edit .env with your API keys (based on pm.tool choice)"
    echo "  3. Authenticate your git provider:"
    echo "     - GitHub: gh auth login"
    echo "     - GitLab (cloud): glab auth login"
    echo "     - GitLab (self-hosted): glab auth login --hostname gitlab.yourcompany.com"
    echo ""
